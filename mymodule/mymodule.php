...
require(dirname(__FILE__).'/mymodule.class.php');
...
class MyModule extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->trans('Adds a new menu to the top of your website.');
    ...
    public function installDb()
    {
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'mymodule` (
			`id_mymodule` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
            Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'mymodule_lang` (
			`id_mymodule` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_mymodule` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
    }
    ...
    {
        Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'mymodule`');
        Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'mymodule_lang`');
        return true;
    }
    ...
    public function getContent()
    {
            ...
            if (Tools::isSubmit('submitBlocktopmenuLinks')) {
                ...
                if ($count_links_label || $count_label) {
                    if (!$count_links_label) {
                        ...
                        foreach ($shops as $shop_id) {
                            $added = MyLink::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)$shop_id);
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('deletemymodule')) {
                $errors_delete_link = array();
                $id_mymodule = Tools::getValue('id_mymodule', 0);
                $shops = Shop::getContextListShopID();

                foreach ($shops as $shop_id) {
                    $deleted = MyLink::remove($id_mymodule, (int)$shop_id);
                    Configuration::updateValue('MOD_BLOCKTOPMENU_ITEMS', str_replace(array('LNK'.$id_mymodule.',', 'LNK'.$id_mymodule), '', Configuration::get('MOD_BLOCKTOPMENU_ITEMS')));
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('updatemymodule')) {
                $id_mymodule = (int)Tools::getValue('id_mymodule', 0);
                ...
                if (Tools::isSubmit('updatelink')) {
                    ...
                    MyLink::update($link, $label, $new_window, (int)$id_shop, (int)$id_mymodule);
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, MyLink::gets((int)$id_lang, null, (int)$shop_id));
        }
    ...
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = MyLink::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = MyLink::get($link[0]['id_mymodule'], (int)$default_language, (int)Shop::getContextShopID());
                        }
                        $html .= '<option selected="selected" value="LNK'.(int)$link[0]['id_mymodule'].'">'.Tools::safeOutput($link[0]['label']).'</option>';
                    }
                    break;
        ...
        foreach ($menu_items as $item) {
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'SHOP':
                    ...
                    break;
                case 'LNK':
                    $link = MyLink::get($id, $id_lang, $id_shop);
                    if (!empty($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = MyLink::get($link[0]['id_mymodule'], $default_language, (int)Shop::getContextShopID());
                        }
                        $root_node['children'][] = $this->makeNode([
                            'type' => 'link',
    ...
    {
        return _PS_CACHE_DIR_ . 'mymodule';
    }
    ...
    {
        $mymodule = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'mymodule
			WHERE id_shop = '.(int)$params['old_id_shop']
            );

        foreach ($mymodule as $id => $link) {
            Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'mymodule (id_mymodule, id_shop, new_window)
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');

            $mymodule[$id]['new_id_mymodule'] = Db::getInstance()->Insert_ID();
        }

        foreach ($mymodule as $id => $link) {
            $lang = Db::getInstance()->executeS('
					SELECT id_lang, '.(int)$params['new_id_shop'].', label, link
					FROM '._DB_PREFIX_.'mymodule_lang
					WHERE id_mymodule = '.(int)$link['id_mymodule'].' AND id_shop = '.(int)$params['old_id_shop']);

            foreach ($lang as $l) {
                Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'mymodule_lang (id_mymodule, id_lang, id_shop, label, link)
					VALUES ('.(int)$link['new_id_mymodule'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
            }
        }
    }
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => (Tools::getIsset('updatemymodule') && !Tools::getValue('updatemymodule')) ?
        ...
        if (Tools::getIsset('updatemymodule') && !Tools::getValue('updatemymodule')) {
        ...
        if (Tools::isSubmit('updatemymodule')) {
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_mymodule');
    ...
    public function renderChoicesSelect()
    {
        ...
        $links = MyLink::gets($this->context->language->id, null, (int)Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = MyLink::get($link['id_mymodule'], $default_language, (int)Shop::getContextShopID());
                if (!in_array('LNK'.(int)$link[0]['id_mymodule'], $items)) {
                    $html .= '<option value="LNK'.(int)$link[0]['id_mymodule'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
                }
            } elseif (!in_array('LNK'.(int)$link['id_mymodule'], $items)) {
                $html .= '<option value="LNK'.(int)$link['id_mymodule'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
            }
        }
        $html .= '</optgroup>';
        $html .= '</select>';
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updatemymodule')) {
            $link = MyLink::getLinkLang(Tools::getValue('id_mymodule'), (int)Shop::getContextShopID());
        ...
        $fields_values = array(
            ...
            'id_mymodule' => Tools::getValue('id_mymodule'),
        );
    ...
    public function renderList()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, MyLink::gets((int)$this->context->language->id, null, (int)$shop_id));
        }
        ...
        $helper->identifier = 'id_mymodule';
        $helper->table = 'mymodule';
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch('module:mymodule/mymodule.tpl');
    }
}
